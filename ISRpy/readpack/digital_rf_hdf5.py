...
$Id: digital_rf_hdf5.py 813 2015-09-10 13:52:24Z brideout $
"""
...
class write_hdf5_channel:
    ...
    def __init__(self, directory, dtype_str, samples_per_file, files_per_directory, start_global_index, sample_rate, uuid_str,
        ...
        Inputs:
            ...
            files_per_directory - number files per subdirectory in form YYYY-MM-DDTHH-MM-SS before new subdirectory created.
                If 0, break subdirectories at hourly boundaries, so all subdirectories in form YYYY-MM-DDTHH-00-00
        ...
        if files_per_directory < 0:
            raise ValueError, 'files_per_directory must not be negative, not %s' % (str(files_per_directory))
    ...
    def _verify_input(self, arr):
        ...
        if self.is_complex:
            ...
            if not arr.dtype.names is None:
    ...
    def get_continuous_vector(self, start_unix_sample, stop_unix_sample, last_update_has_full_metadata=False):
        ...
        if last_update_has_full_metadata:
            ...
            if self._last_start_sample:
                if start_unix_sample > self._last_start_sample and \
                    stop_unix_sample <= self._last_start_sample + self.samples_per_file:
            ...
            for file_to_search in files_to_search:
                arr = self._read_data_from_file(file_to_search, start_unix_sample, stop_unix_sample, ret_array)
    ...
    def _get_files_to_search(self, start_unix_sample, stop_unix_sample):
        ...
        for i in range(first_index, len(self.sub_directory_recarray)):
            ...
            for second in seconds_list:
                glob_str = os.path.join(self.sub_directory_recarray['subdirectory'][i],
                                        'rf@%i.???.h5' % (second))
